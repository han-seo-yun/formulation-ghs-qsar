...
#
# 설계 결함 11열 제외 (2026-09-21 총책임자 결정, `lib_model.FEAT_DROP`)
#   `L2단독` 42열 전수 점검에서 확인한 3건이다. 죽은 열 1(`ct_not_applicable`,
#   최빈 98.45% · 중요도 0.0%) · 완전 중복쌍의 한쪽 4(`ct_{eye,skin,sens}_ord` 는
#   `f_ct_*_ord` 와, `f_pct_active` 는 `f_pct_active_presumed` 와 결측 위치까지
#   값이 같다) · 감작 CT 6. 감작 CT 는 라벨이 아니라 혼합물 가산 디스크립터지만,
#   감작을 학습에 쓰지 않는다는 원칙과 비대칭이라 피처에서도 뺀다.
#
#   **노선 판정표(`피처노선_판정.csv`)에는 그대로 남긴다** — 어느 열이 어느 정보원에
#   속하는지는 감사 기록이고, 여기서 빼는 것은 arm 의 대표 자격뿐이다. 비용은
#   `04_모델산출물/v8_노선2정리/` 에서 노선2 규약으로 측정했다: 중복 4쌍과 죽은 열은
#   두 경로 모두 무비용, 감작 CT 는 눈 MCC −0.062(문서독립 층 −0.045)의 실측 손실이
#   있다. 게이트 판정은 전후 모두 미달로 바뀌지 않는다.
def cols(*lanes):
    return sorted(c for c in TR[TR["노선"].isin(lanes)]["열"]
                  if c not in set(L.FEAT_DROP_ALL))
...
# 제외가 실제로 먹었는지 arm 쪽에서 직접 본다. `cols()` 의 필터는 이름 대조이므로
# `FEAT_DROP` 에 오타가 나거나 manifest 에서 열 이름이 바뀌면 조용히 no-op 이 되고
# arm 이 원래 크기로 되돌아간다. `lib_model.featset()` 에는 이 가드가 있지만 동결
# `run_model_lane2.py` 는 arm json 을 직접 읽어 `featset()` 을 거치지 않으므로,
# 그 경로의 유일한 가드는 여기다.
_drop = set(L.FEAT_DROP_ALL)
_train = set(TR["열"])
assert _drop <= _train, f"제외 대상이 학습 열에 없다: {sorted(_drop - _train)}"
assert len(ARMS["L2단독"]) == N["L2"] - len(_drop), \
    f"L2단독 {len(ARMS['L2단독'])}열 != L2 귀속 {N['L2']} − 제외 {len(_drop)}"
# 결합 노선에는 제외 대상이 없다는 현재 상태를 못 박는다. 장래에 결합 열이 제외
# 대상이 되면 `L2L3결합_열` 만 줄고 `노선_열수` 는 28 로 남아 json 안이 어긋난다.
assert len(cols("L2L3결합")) == N["L2L3결합"], \
    "L2L3결합 에 제외 대상이 생겼다 — json 의 노선_열수·L2L3결합_이유 서술도 같이 고쳐라"
log(f"설계 결함 {len(_drop)}열을 모든 arm 에서 제외 (노선 판정표에는 유지)")
...
with open(OUT / "피처노선_arm.json", "w", encoding="utf-8") as f:
    json.dump({
        ...
        # `노선_열수` 는 노선 귀속(감사 기록), `arm` 은 실제 학습 열이다. 아래 제외
        # 때문에 두 숫자가 다르다 — 예: L2 귀속 42열, `L2단독` arm 31열.
        "피처정리_제외": {
            "결정": "2026-09-21 총책임자. `L2단독` 42열 전수 점검에서 확인한 설계 결함",
            "출처": "lib_model.FEAT_DROP",
            "열": {k: list(v) for k, v in L.FEAT_DROP.items()},
            "열수": len(L.FEAT_DROP_ALL),
            "적용범위": "모든 arm. 노선 판정표(피처노선_판정.csv)에는 그대로 남긴다 — "
                    "노선 귀속은 감사 기록이고 여기서 빼는 것은 대표 자격뿐이다",
            "실측비용": "04_모델산출물/v8_노선2정리/. 중복쌍·죽은열은 무비용, 감작 CT 는 "
                    "눈 MCC −0.062(문서독립 층 −0.045). AUC 게이트 판정은 전후 모두 미달",
            "비용측정한_arm": ["L2단독"],
            "비용_미측정_arm": ["L2+결합", "L2+L1", "L2+L3", "전체_노선혼합"],
            "미측정_주의": "제외는 총책임자 결정으로 8개 arm 전부에 적용했지만, 비용을 "
                     "실측한 것은 `L2단독` 뿐이다. 나머지 4개(점증 ablation 부가 행)의 "
                     "제외 비용은 재실행 결과로 확인해야 하며, `L2단독` 수치를 "
                     "그쪽에 옮겨 적으면 안 된다",
        },
