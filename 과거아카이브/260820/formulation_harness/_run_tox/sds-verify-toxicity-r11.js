export const meta = {
  name: 'sds-verify-toxicity-r11',
  description: 'Independent re-check of toxicity research claims (found items only), isolated copy for chunk 11.',
  phases: [
...
Cited source: ${item.source_url || 'none recorded - original hint (if any) was used, verify from scratch'}
...
const finalResults = pass1Clean.map(({ item, v }) => {
  ...
  return {
    ...item,
    field_values: mismatch ? {} : newFieldValues,
    resolution: p2.v2.corrected_resolution || (mismatch ? 'ingredient_mismatch' : item.resolution),
